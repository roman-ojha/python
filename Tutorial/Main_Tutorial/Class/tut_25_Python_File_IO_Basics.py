...
3) "a" -> Add more content to a file
4) "t" -> text mode -> default mode
5) "b" -> binary mode
5) "+" -> read and write 
 """
...
""" 
   ...
   b : b stands for binary and this mode can only open the binary files, that are read in bytes. The binary files include images, documents, or    
   all other files that require specific software to be read.
